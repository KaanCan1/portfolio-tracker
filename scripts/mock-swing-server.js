...
const oppFix = (symbol, name, score, setupType, entry, stop, target, rr, owned, watched) => ({
  ...
  verdict: { key: "buy", tone: "good", label: "Girilebilir" }, grade: score >= 80 ? "A" : "B",
  ...
  pattern: null, weekly: { dir: "up", tone: "good" },
  hitRate: score >= 80 ? { winRate: 62, n: 13, avgR: 0.7 } : null,
  ret1M: 6.2, ret3M: 18.4, fromHighPct: 4.1,
  spark: Array.from({ length: 30 }, (_, i) => +(entry * (0.86 + 0.005 * i + 0.02 * Math.sin(i / 3))).toFixed(2)),
  insider: symbol === "MU" ? { buys: 3, sells: 0, buyValue: 2.1e6, netValue: 2.1e6, lastBuy: iso(Y, M, 8), signal: "buy" } : null,
  news: symbol === "NVDA" ? [{ headline: "Yeni veri merkezi anlaşması duyuruldu (MOCK)", url: "https://example.com", datetime: Date.now() / 1000 - 86400, source: "Mock" }] : [],
  why: [{ tone: "good", text: "50g üstünde, hacim teyitli kırılım bölgesi. (MOCK)" }, { tone: "warn", text: "Bilanço 9 gün sonra — pozisyon taşıma riskini planla." }],
});
